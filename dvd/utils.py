...
def call_openai_model_with_tools(
    ...
    if tools:
        llm = llm.bind_tools(tools=tools, tool_choice=tool_choice)
    ...
    if image_paths:
        ...
        image_data_list = [
            local_image_to_data_url(image_path) for image_path in image_paths
        ]
        human_message = []
        for mime_type, b64_image in image_data_list:
    ...
    #     # TODO: maybe with langchain we can always return the whole message
    #     return {"content": response.content.strip(), "tool_calls": None}

    # TODO: check if this is enough with langchain
    return response.model_dump()
